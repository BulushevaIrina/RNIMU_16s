...
def downsampling_filtration(input1, input2, number_of_reads, number_of_max_mismatches, out_file, umi_or_not, pr1, pr2):
    ...
    with gzip.open(input1, 'rb') as f:
        number_of_lines1=0
        for i, l in enumerate(f):
            pass
            number_of_lines1+=1
    print('\n\nFirst read file contains ', int(number_of_lines1/4),' reads' )
    f.close()
    with gzip.open(input2, 'rb') as f:
        number_of_lines2=0
        for i, l in enumerate(f):
            pass
            number_of_lines2+=1
    print('Second read file contains ', int(number_of_lines2/4),' reads' )
    f.close()

    if int(number_of_lines1/4)!=int(number_of_lines2/4):
        print("ERROR DIFFERENT NUMBER OF READS")

    if number_of_reads>int(number_of_lines1/4):
        number_of_reads=int(number_of_lines1/4)
    ...
    print('primers ',pr1,' ',pr2,'\n')
    
    if os.path.exists(filtr_output_1):
        os.remove(filtr_output_1)
    if os.path.exists(filtr_output_2):
        os.remove(filtr_output_2)

    for i in range (0, number_of_reads):

        current_read1=content1[i*4:i*4+4]
        current_read2=content2[i*4:i*4+4]
        #print(i,i*4,i*4+4,len(current_read1),'\n\n', len(current_read2))
        #if len(current_read1[1])<60:
        #    print (current_read1[1])
        ...
        #f_out_1 = open(filtr_output_1, 'w')
        #f_out_2 = open(filtr_output_2, 'w')
    ...
    output_file.write('primers: '+pr1+' '+pr2+'\n\n')

    output_file.write('First read file contains '+str(int(number_of_lines1/4))+' reads\n' )
    output_file.write('Second read file contains '+str(int(number_of_lines2/4))+' reads\n' )
    ...
    return filtr_output_1, filtr_output_2
